...
let perguntasRespondidas = 0; //Conta o número de perguntas respondidas corretamente
    ...
    if (selectedOption) {
      ...
      if (userAnswer === pergunta.respostaCorreta) {
        ...
        if (perguntasRespondidas === 3) {
          ...
          updateUserPoints();
          document
            .getElementById("porta")
            .setAttribute("href", "../menuES.html");
        ...
        document.getElementById(`area${index}`).removeAttribute("data-index");
      } else {
...
function updateUserPoints() {
  ...
  if (currentUser) {
    currentUser.pontos = (currentUser.pontos || 0) + pontos; // Adiciona os pontos ao total do utilizador
        ...
        if (premio.id) {
          ...
          if (premioLocal) {
            ...
            if (
              ...
              // Atualiza com os novos dados de currentUser
              sessionStorage.setItem("loggedUser", JSON.stringify(currentUser));

              // Atualiza users.premios
              if (userIndex !== -1) {
